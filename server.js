...
io.on('connection', socket => {
  ...
  socket.on('join', token=>{
    ...
    if(waitingUsers.length>=2){
      ...
      user1.join(room); user2.join(room);
      user1.room = room; user2.room = room;
  ...
  socket.on('sendMessage', msg=>{
    if(socket.room && msg.id && msg.text){
      const chatMsg={id:msg.id,sender:socket.userName,text:msg.text,time:new Date().toISOString(),reactions:{}};
  ...
  socket.on('sendVoice', data=>{
    if(socket.room && data.id){
      const chatMsg={id:data.id,sender:socket.userName,url:data.url,duration:data.duration,time:new Date().toISOString(),reactions:{}};
      messages.push(chatMsg);
  ...
  socket.on('leave', ()=>{
    if(socket.room){ socket.to(socket.room).emit('partner_left'); socket.leave(socket.room); socket.room=null; }
    const idx = waitingUsers.indexOf(socket);
    if(idx!==-1) waitingUsers.splice(idx,1);
    if(socket.counted){ connectedUsers--; io.emit('user_count',connectedUsers); }
  });
